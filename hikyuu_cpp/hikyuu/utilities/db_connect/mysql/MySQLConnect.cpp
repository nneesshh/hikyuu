    ...
    void MySQLConnect::connect() {
        try {
            ...
            for ( auto i = 0; i < maxRetry; i++ ) {
                auto* h = mysql_real_connect( m_mysql, host.c_str(), usr.c_str(), pwd.c_str(),
                                              database.c_str(), port, NULL, CLIENT_MULTI_STATEMENTS );
                if ( h != NULL ) {
                    success = true;
                    if ( i > 0 ) {
                        HKU_INFO(
                            "({}/{})[MySQLConnect::connect] connect to database success -- host={} port={} usr={} pwd={} database={}",
                            i + 1, maxRetry, host, port, usr, pwd, database );
                    }
                    break;
                }
                HKU_ERROR( "({}/{})[MySQLConnect::connect] connect to database failed! host={} port={} usr={} pwd={} database={} retry after 3 seconds",
                           i + 1, maxRetry, host, port, usr, pwd, database );
                std::this_thread::sleep_for( std::chrono::seconds( 3 ) );
            }
            if ( !success ) {
                throw SQLException( mysql_errno( m_mysql ), fmt::format( "Failed to connect to database! err {} -- host={} port={} usr={} pwd={} database={}", mysql_error( m_mysql ), host, port, usr, pwd, database ) );
            }
